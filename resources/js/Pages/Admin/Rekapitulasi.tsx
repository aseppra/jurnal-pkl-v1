...
interface Props { students: PaginatedStudents; stats: { totalSiswa: number; totalDudi: number }; classes: string[]; companies: string[]; filters: { search?: string; class?: string; company?: string }; }
    ...
    return (
        <AdminLayout title="Rekapitulasi Jurnal" subtitle="Monitoring Kehadiran dan Kelengkapan Jurnal Siswa">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: 'Total Siswa PKL', value: stats.totalSiswa, trend: '100% Aktif', icon: 'trending_up', color: 'emerald' },
                    { label: 'Total Perusahaan', value: stats.totalDudi, trend: 'Mitra Terdaftar', icon: 'corporate_fare', color: 'slate' },
                ].map((s, i) => (
                    <div key={i} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                        <p className="text-slate-500 text-sm font-medium">{s.label}</p>
                        <p className="text-2xl font-bold mt-2">{s.value}</p>
                        <div className={`flex items-center gap-1 text-${s.color}-500 text-xs mt-2`}>
                            <span className="material-symbols-outlined text-xs">{s.icon}</span><span>{s.trend}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead><tr className="bg-slate-50 text-slate-600 text-xs font-bold uppercase tracking-wider">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {students.data.map((s) => (
                                <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="size-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-xs">{s.name.split(' ').map(n => n[0]).join('')}</div><div><p className="text-sm font-semibold">{s.name}</p><p className="text-xs text-slate-500">NIS: {s.nisn}</p></div></div></td>
                                    <td className="px-6 py-4 text-sm">{s.class}</td>
                                    <td className="px-6 py-4 text-sm font-medium">{s.company}</td>
                                    <td className="px-6 py-4"><div className="flex items-center gap-2"><span className="text-sm font-semibold">{s.attendance}</span><div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${s.statusColor === 'emerald' ? 'bg-emerald-500' : s.statusColor === 'amber' ? 'bg-amber-500' : 'bg-rose-500'}`} style={{ width: `${s.progress}%` }}></div></div></div></td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link href={route('admin.rekapitulasi.show', s.id)} className="inline-flex items-center justify-center bg-slate-50 text-slate-600 hover:text-primary hover:bg-primary/10 border border-slate-200 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
                                            Lihat Detail
                                        </Link>
                                    </td>
                                </tr>
